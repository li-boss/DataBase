...
bool DDLExecutor::createTable(const std::string& tableName, const std::vector<FieldDefinition>& fields) {
    ...
    std::vector<FieldDefinition> alignedFields = fields;
    ...
    for (const auto& field : alignedFields) {
        ...
        tdfOffset += sizeof(FieldDefinition);
    }
